...
{
    glfwSetInputMode(
        m_WindowPtr, GLFW_CURSOR, showCursor ? GLFW_CURSOR_NORMAL : GLFW_CURSOR_HIDDEN);
}
...
void Window::updateViewport() const { GLCall(glViewport(0, 0, m_Width, m_Height)); }
...
void Window::defineEvents()
{
    EventHandler::addEvent(Event(
        [&]()
        {
            ...
            std::string alteredWindowTitle
                = std::string(std::string(m_Title) + " | Fps: " + std::to_string(m_FpsCount));
