...
Window::Window(const ProjectConfig& projectConfig)
    : m_GameWindow(projectConfig), m_Width(projectConfig.ScreenResolution.x),
...
void Window::onRender() { m_GameWindow.onRender(); }
...
void Window::showCursor(bool showCursor) {
    glfwSetInputMode(m_WindowPtr, GLFW_CURSOR,
                     showCursor ? GLFW_CURSOR_NORMAL : GLFW_CURSOR_HIDDEN);
}

void Window::showFps(bool showFps) { m_ShowFps = showFps; }
...
void Window::flipTexturesH(bool flip) { Texture2D::flipTexturesH(flip); }
...
void Window::defineEvents() {
    EventHandler::addEvent(Event(
        [&]() {
            ...
            std::string alteredWindowTitle = std::string(
                std::string(m_Title) + " | Fps: " + std::to_string(m_FpsCount));
...
glm::ivec2 Window::getFrameSize() const { return m_GameWindow.getFrameSize(); }

FrameBuffer& Window::getFrameBuffer() { return m_GameWindow.getFrameBuffer(); }
} // namespace davincpp
