...
void FrameBuffer::setPixel(int pixelX, int pixelY, glm::vec4 color)
{
    ...
    m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + R)] = static_cast<GLubyte>(
        alphaChl1 * color.r
        + alphaChl2 * (1.0f - alphaChl1) * static_cast<float>(m_FrameBuffer.get()[pixelIdx + R]));
    m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + G)] = static_cast<GLubyte>(
        alphaChl1 * color.g
        + alphaChl2 * (1.0f - alphaChl1) * static_cast<float>(m_FrameBuffer.get()[pixelIdx + G]));
    m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + B)] = static_cast<GLubyte>(
        alphaChl1 * color.b
        + alphaChl2 * (1.0f - alphaChl1) * static_cast<float>(m_FrameBuffer.get()[pixelIdx + B]));
    m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + A)]
        = static_cast<GLubyte>((alphaChl1 + alphaChl2 * (1 - alphaChl1)) * MAX_COLOR_VALUE);
}
...
size_t FrameBuffer::getPixelCount() const { return m_FrameWidth * m_FrameHeight; }

glm::uvec2 FrameBuffer::getPixelSize() const { return { m_PixelSizeX, m_PixelSizeY }; }

glm::uvec2 FrameBuffer::getFrameSize() const { return { m_FrameWidth, m_FrameHeight }; }

std::shared_ptr<GLubyte[]> FrameBuffer::getBufferPtr() const { return m_FrameBuffer; }

inline bool FrameBuffer::isPixelInBoundry(int pixelX, int pixelY) const
{
    return pixelX >= 0 && pixelX < static_cast<int>(m_FrameWidth) && pixelY >= 0
           && pixelY < static_cast<int>(m_FrameHeight);
}
