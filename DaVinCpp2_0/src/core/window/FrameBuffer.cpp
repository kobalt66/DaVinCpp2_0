...
namespace davincpp {
FrameBuffer::FrameBuffer(uint32_t pixelSizeX, uint32_t pixelSizeY,
                         uint32_t bytesPerPixel)
    : m_PixelSizeX(pixelSizeX), m_PixelSizeY(pixelSizeY),
      m_BytesPerPixel(bytesPerPixel) {}

void FrameBuffer::onResize(uint32_t windowSizeX, uint32_t windowSizeY) {
    if (windowSizeX < MIN_FRAMEBUFFER_SIZEX ||
        windowSizeY < MIN_FRAMEBUFFER_SIZEY) {
        return;
    }
    ...
    m_FrameBuffer.reset();
    m_FrameBuffer = std::make_shared<GLubyte[]>(m_FrameWidth * m_FrameHeight *
                                                m_BytesPerPixel);
}
...
void FrameBuffer::onClear() {
    ...
    memset(m_FrameBuffer.get(), 0,
           m_FrameWidth * m_FrameHeight * m_BytesPerPixel);
}
...
void FrameBuffer::setPixel(int pixelX, int pixelY, glm::vec4 color) {
    ...
    float alphaChl2 =
        static_cast<float>(m_FrameBuffer.get()[pixelIdx + A]) / MAX_COLOR_VALUE;

    m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + R)] = static_cast<GLubyte>(
        alphaChl1 * color.r +
        alphaChl2 * (1.0f - alphaChl1) *
            static_cast<float>(m_FrameBuffer.get()[pixelIdx + R]));
    m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + G)] = static_cast<GLubyte>(
        alphaChl1 * color.g +
        alphaChl2 * (1.0f - alphaChl1) *
            static_cast<float>(m_FrameBuffer.get()[pixelIdx + G]));
    m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + B)] = static_cast<GLubyte>(
        alphaChl1 * color.b +
        alphaChl2 * (1.0f - alphaChl1) *
            static_cast<float>(m_FrameBuffer.get()[pixelIdx + B]));
    m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + A)] = static_cast<GLubyte>(
        (alphaChl1 + alphaChl2 * (1 - alphaChl1)) * MAX_COLOR_VALUE);
}
...
glm::vec4 FrameBuffer::getPixel(int pixelX, int pixelY) const {
    ...
    return glm::vec4(m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + R)],
...
inline bool FrameBuffer::isPixelInBoundry(int pixelX, int pixelY) const {
    return pixelX >= 0 && pixelX < static_cast<int>(m_FrameWidth) &&
           pixelY >= 0 && pixelY < static_cast<int>(m_FrameHeight);
}
