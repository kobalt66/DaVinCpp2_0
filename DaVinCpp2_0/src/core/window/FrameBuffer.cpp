...
void FrameBuffer::setPixel(int pixelX, int pixelY, glm::vec4 color)
{
	...
	m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + R)] = static_cast<GLubyte>(
	    alphaChl1 * color.r + alphaChl2 * (1.0f - alphaChl1) * static_cast<float>(m_FrameBuffer.get()[pixelIdx + R]));
	m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + G)] = static_cast<GLubyte>(
	    alphaChl1 * color.g + alphaChl2 * (1.0f - alphaChl1) * static_cast<float>(m_FrameBuffer.get()[pixelIdx + G]));
	m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + B)] = static_cast<GLubyte>(
	    alphaChl1 * color.b + alphaChl2 * (1.0f - alphaChl1) * static_cast<float>(m_FrameBuffer.get()[pixelIdx + B]));
...
glm::vec4 FrameBuffer::getPixel(int pixelX, int pixelY) const
{
	...
	return glm::vec4(
	    m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + R)], m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + G)],
	    m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + B)], m_FrameBuffer[static_cast<ptrdiff_t>(pixelIdx + A)]);
}
