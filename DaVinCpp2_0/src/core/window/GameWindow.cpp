...
void GameWindow::onSetup()
{
    ...
    m_WindowShader = Shader(
        ShaderProfile(VertexAttribute(0, 2, GL_FLOAT, 0), VertexAttribute(1, 2, GL_FLOAT, 2)));
...
void GameWindow::onMousePosition(double xpos, double ypos)
{
    ...
    m_MousePosition.y = m_FrameBuffer.getFrameSize().y
                        - static_cast<int>(ypos / m_FrameBuffer.getPixelSize().y) - 1;
}

glm::ivec2 GameWindow::getFrameSize() const { return m_FrameBuffer.getFrameSize(); }
