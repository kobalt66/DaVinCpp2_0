...
namespace davincpp {
Square::Square(glm::vec2 position, glm::vec4 color, float width, float height,
               bool centerAligned)
    : Renderable(position, color), m_Width(width), m_Height(height),
      m_Scale(width, height), m_CenterAligned(centerAligned) {}

void Square::onRender(const GameObjectStats& gameObjectStats,
    ...
    if (texture) {
        surfaceScale = texture->wrapToSurface()
                           ? surfaceScale
                           : static_cast<glm::vec2>(texture->getTextureSize());
    }
    ...
    for (int y = flooredStartPosition.y; y < flooredEndPosition.y; y++) {
        for (int x = flooredStartPosition.x; x < flooredEndPosition.x; x++) {
            frameBuffer.setPixel(x, y,
                                 texture ? mapTextureByUVCoords(texture, x, y,
                                                                surfaceScale.x,
                                                                surfaceScale.y)
                                         : m_Color);
        }
...
glm::vec2 Square::getScale() const { return m_Scale; }
} // namespace davincpp
