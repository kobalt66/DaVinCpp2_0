...
namespace davincpp
{
Square::Square(glm::vec2 position, glm::vec4 color, float width, float height, bool centerAligned)
...
void Square::onRender(const GameObjectStats& gameObjectStats, FrameBuffer& frameBuffer) const
{
    ...
    if (texture)
    {
        surfaceScale = texture->wrapToSurface() ? surfaceScale
    ...
    for (int y = flooredStartPosition.y; y < flooredEndPosition.y; y++)
    {
        for (int x = flooredStartPosition.x; x < flooredEndPosition.x; x++)
        {
            frameBuffer.setPixel(
                x,
                y,
                texture ? mapTextureByUVCoords(texture, x, y, surfaceScale.x, surfaceScale.y)
                        : m_Color);
        }
    }
}
