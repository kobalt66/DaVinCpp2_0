...
void Circle::drawLine(FrameBuffer& frameBuffer, std::shared_ptr<Texture2D> texture, int sx, int ex, int y,
                      glm::vec2 surfaceScale) const
{
	for (int x = sx; x <= ex; x++) {
		...
		frameBuffer.setPixel(
		    x, y,
		    texture ? mapTextureByUVCoords(texture, textureOffsetX, textureOffsetY, surfaceScale.x, surfaceScale.y)
		            : m_Color);
	}
}
} // namespace davincpp
