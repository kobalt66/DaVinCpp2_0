...
void Renderable::onRender(const GameObjectStats& gameObjectStats,
                          FrameBuffer&           frameBuffer) const {
    throw not_implemented(__LINE__, __FILE__);
}

glm::vec2 Renderable::getPosition() { return m_Position; }

void Renderable::setPosition(glm::vec2 position) { m_Position = position; }

glm::vec4 Renderable::mapTextureByUVCoords(std::shared_ptr<Texture2D> texture,
                                           int pixelX, int pixelY, float width,
                                           float height) const {
