...
namespace davincpp {
GameObject::GameObject(std::string objectName) : m_Stats(this, objectName) {}
...
void GameObject::setComponent(Component* component) {
    if (component->getUniqueType() == ComponentType::NONE) {
        throw davincpp_error(Console::fmtTxt(
            "Failed to add component to GameObject(", m_Stats.m_ObjectName,
            "): component has not valid type associated!"));
    }

    if (m_Components.contains(component->getUniqueType())) {
        throw davincpp_error(Console::fmtTxt(
            "Failed to add component to GameObject(", m_Stats.m_ObjectName,
            "): There is already a component with the same type attached to "
            "this game object!"));
    }
...
GameObject::getComponent(ComponentType uniqueType) const {
...
void GameObject::onLoad() {
    for (std::pair<ComponentType, std::shared_ptr<Component>> component :
         m_Components) {
...
void GameObject::onRender(FrameBuffer& frameBuffer) const {
    for (const std::pair<ComponentType, std::shared_ptr<Component>> component :
         m_Components) {
...
void GameObject::setName(const char* name) { m_Stats.m_ObjectName = name; }

std::string GameObject::getName() const { return m_Stats.m_ObjectName; }

std::shared_ptr<Texture2D> GameObject::getTexture() const {
    return std::dynamic_pointer_cast<Texture2D>(m_Texture.lock());
}

GameObjectStats& GameObject::getStats() { return m_Stats; }
} // namespace davincpp
