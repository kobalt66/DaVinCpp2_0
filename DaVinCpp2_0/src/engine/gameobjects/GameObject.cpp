...
void GameObject::setComponent(Component* component)
{
    ...
    {
        throw davincpp_error(Console::fmtTxt("Failed to add component to GameObject(",
                                             m_Stats.m_ObjectName,
    ...
    if (m_Components.contains(component->getUniqueType()))
    {
        throw davincpp_error(Console::fmtTxt(
            "Failed to add component to GameObject(",
            m_Stats.m_ObjectName,
            "): There is already a component with the same type attached to this game object!"));
    }
