...
void GameObject::setComponent(Component* component)
{
	if (component->getUniqueType() == ComponentType::NONE) {
		throw davincpp_error(Console::fmtTxt("Failed to add component to GameObject(", m_Stats.m_ObjectName,
		                                     "): component has not valid type associated!"));
	}

	if (m_Components.contains(component->getUniqueType())) {
		throw davincpp_error(
		    Console::fmtTxt("Failed to add component to GameObject(", m_Stats.m_ObjectName,
