...
class Component {
  public:
    ...
    virtual void onRender(const GameObjectStats& gameObjectStats,
                          FrameBuffer&           frameBuffer) const {}
    virtual void onUpdate(GameObjectStats& gameObjectStats) {}
    virtual void onShutdown(GameObjectStats& gameObjectStats) {}

    /// TODO:	Add virtual functions that allow the components to get properly
    /// rendered on the game engine's UI.
    ///			Every component has to be able to get rendered differently
    ///inside the engine's UI.
