...
void GameObjectManager::onLoad() {
    for (const std::pair<std::string, std::shared_ptr<GameObject>>&
             gameObjectEntry : m_GameObjects) {
...
void GameObjectManager::onShutdown() { m_GameObjects.clear(); }

void GameObjectManager::registerGameObject(GameObject* gameObject) {
    validateGameObjectNaming(gameObject);
    m_GameObjects[gameObject->getName()] =
        std::shared_ptr<GameObject>(gameObject);
}

std::shared_ptr<GameObject>
GameObjectManager::getGameObject(std::string_view objectName) {
