...
void Application::onStartEngine()
{
    ...
    ASSERT_ENGINE_CALL(m_ProjectManager
                       = std::make_shared<ProjectManager>("[TEST_ENV]/davincpp.yaml"),
...
void Application::onLoad()
{
    ...
    ASSERT_ENGINE_CALL(m_GameObjectManager = std::make_unique<GameObjectManager>(),
    ...
    // square3->setComponent(new Square(glm::vec2(10, 59), glm::vec4(100.0f, 12.0f, 43.0f,
    // 200.0f), 10.0f, 10.0f, true)); m_GameObjectManager->registerGameObject(square3);
    //
    // GameObject* triangle = new GameObject("Triangle");
    // triangle->setComponent(new Triangle(glm::vec2(200, 0), glm::vec2(20, 5), glm::vec2(0, 50),
    // RED)); triangle->setComponent(new Texture2D("[BASE_DIR]src/data/Cursor.png", true));
    // m_GameObjectManager->registerGameObject(triangle);

    auto triangle2 = new GameObject("Triangle");
    triangle2->setComponent(
        new Triangle(glm::vec2(200, 50), glm::vec2(200, 100), glm::vec2(300, 50), RED));
    ...
    triangle3->setComponent(
        new Triangle(glm::vec2(200, 100), glm::vec2(250, 100), glm::vec2(350, 50), RED));
...
{
    ASSERT_ENGINE_CALL(m_GameObjectManager->onRender(m_Window->getFrameBuffer()),
...
void Application::loadSelectedProject()
{
    ...
    if (projectIdx >= m_ProjectManager->getProjectList().size())
    {
        throw davincpp_error(
            "Failed to load DaVinCpp project: project index out of project list range!");
    }

    m_DaVinCppProject
        = std::make_shared<Project>(m_ProjectManager->getProjectList().at(projectIdx));
