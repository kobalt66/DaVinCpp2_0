...
namespace davincpp
{
SelectFilepathButton::SelectFilepathButton(const std::string& displayText,
...
{
    Console::printCenteredText(m_FilePath.empty() ? m_DisplayText : m_FilePath.c_str(),
...
{
    std::shared_ptr<FilepathSelectionWizard> filepathSelectionMenu
        = selectionMenu->getMenuPage<FilepathSelectionWizard>(SelectionMenu::PAGE_SELECT_FILE_PATH);
...
std::filesystem::path SelectFilepathButton::getFilePath() const { return m_FilePath; }
}  // namespace davincpp
