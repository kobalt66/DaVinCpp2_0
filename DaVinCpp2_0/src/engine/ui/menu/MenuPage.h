...
class MenuPage {
  public:
    ...
    getMenuElementByTag(std::string_view uniqueTag) const {
        if (m_MenuElementsMap.find(uniqueTag.data()) ==
            m_MenuElementsMap.end()) {
            throw davincpp_error(Console::fmtTxt(
                "Failed to fetch menu element with unique tag '", uniqueTag,
                "': The element doesn't exist!"));
        }

        if (std::shared_ptr<T> menuElement = std::dynamic_pointer_cast<T>(
                m_MenuElementsMap.at(uniqueTag.data()))) {
            return menuElement;
        }

        throw davincpp_error(Console::fmtTxt(
            "Failed to fetch menu element with unique tag '", uniqueTag,
            "': Failed to cast menu element to the desired menu element "
            "type!"));
    }
  ...
  protected:
    const char* m_Title;
    std::unordered_map<std::string, std::shared_ptr<MenuElement>>
                                              m_MenuElementsMap;
