...
void SelectionMenu::onLoad()
{
	...
	m_MenuPages[PAGE_MAIN] = std::make_shared<MenuPage>(
	    "D a V i n C p p  2.0\nMain Menu", std::make_shared<BreakElement>(), std::make_shared<PlayButton>("Start Game"),
	    std::make_shared<BreakElement>(), std::make_shared<PageElement>("Select project", PAGE_SELECT_PROJECT),
	...
	m_MenuPages[PAGE_SELECT_PROJECT] =
	    std::make_shared<MenuPage>("Select a project", std::make_shared<PageElement>("../", PAGE_MAIN));
	...
	m_MenuPages[PAGE_CREATE_PROJECT] = std::make_shared<ProjectCreationWizard>(
	    "Create a new project", std::make_shared<BreakElement>(), std::make_shared<PageElement>("<Cancle>", PAGE_MAIN),
	    ...
	    std::make_shared<SwitchElement>("Show system cursor", ProjectCreationWizard::PROJECT_SHOW_CURSOR),
	    std::make_shared<BreakElement>(),
	    std::make_shared<NumberFieldElement>(
	        "Resolution (width)", 0, 3000,
	        ProjectCreationWizard::PROJECT_RESOLUTION_WIDTH), // TODO: try to find out what the maximum client size of
	                                                          // screen is!
	    std::make_shared<NumberFieldElement>("Resolution (height)", 0, 3000,
	                                         ProjectCreationWizard::PROJECT_RESOLUTION_HEIGHT),
	    std::make_shared<NumberFieldElement>("Pixel width", 0, 20, ProjectCreationWizard::PROJECT_PIXEL_WIDTH),
	    std::make_shared<NumberFieldElement>("Pixel height", 0, 20, ProjectCreationWizard::PROJECT_PIXEL_HEIGHT),
	    std::make_shared<BreakElement>(), std::make_shared<BreakElement>(),
	    std::make_shared<CreateProjectButton>("<Create project>", actionCreateProject));

	m_MenuPages[PAGE_DELETE_PROJECT] =
	    std::make_shared<MenuPage>("Delete a project", std::make_shared<PageElement>("../", PAGE_MAIN));

	m_MenuPages[PAGE_RENAME_PROJECT] =
	    std::make_shared<MenuPage>("Rename a project", std::make_shared<PageElement>("../", PAGE_MAIN));

	m_MenuPages[PAGE_EDIT_PROJECT_CONFIGS] =
	    std::make_shared<MenuPage>("Edit project configs", std::make_shared<PageElement>("../", PAGE_MAIN));
...
void SelectionMenu::switchPage(std::string_view pageTag)
{
	if (!m_MenuPages.contains(pageTag.data())) {
		Console::printCenteredText(Console::fmtTxt("Failed to switch pages: Page ", pageTag, "doesn't exist!"),
...
std::shared_ptr<T> SelectionMenu::getMenuPage(std::string_view pageTag) const
{
	if (!m_MenuPages.contains(pageTag.data())) {
		throw davincpp_error(
		    Console::fmtTxt("Faile to get menu page called '", pageTag, "': The menu page is not registered!"));
	}
	...
	throw davincpp_error(Console::fmtTxt("Failed to get menu page called '", pageTag,
...
void SelectionMenu::actionCreateProject(SelectionMenu* selectionMenu, ActionButton* buttonRef)
{
	if (auto createProjectButton = dynamic_cast<CreateProjectButton*>(buttonRef)) {
		...
		try {
			projectConfigFilePath = selectionMenu->m_ProjectManager->initProject(projectConfig, projectDirectory);
		} catch (std::runtime_error& error) {
			selectionMenu->setInputControl(false);
			displayDescription(Console::fmtTxt("Failed to created new project called '", projectConfig.ProjectName,
			                                   "':", error.what(), " "),
		...
		selectionMenu->setInputControl(false);
		displayDescription(Console::fmtTxt("Created new project '", projectConfig.ProjectName, "' (Project config at '",
...
template std::shared_ptr<ProjectCreationWizard> SelectionMenu::getMenuPage(std::string_view pageTag) const;
template std::shared_ptr<FilepathSelectionWizard> SelectionMenu::getMenuPage(std::string_view pageTag) const;
