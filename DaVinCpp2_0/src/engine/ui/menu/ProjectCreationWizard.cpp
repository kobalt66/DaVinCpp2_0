...
const char* ProjectCreationWizard::PROJECT_SUPPORTED_VERSION =
    "project_supported_version";
...
const char* ProjectCreationWizard::PROJECT_RESOLUTION_WIDTH =
    "project_resolution_width";
const char* ProjectCreationWizard::PROJECT_RESOLUTION_HEIGHT =
    "project_resolution_height";
const char* ProjectCreationWizard::PROJECT_PIXEL_WIDTH = "project_pixel_width";
const char* ProjectCreationWizard::PROJECT_PIXEL_HEIGHT =
    "project_pixel_height";
...
ProjectCreationWizard::castProjectData() const {
    try {
        return {
            getMenuElementByTag<SelectFilepathButton>(PROJECT_NAME)
                ->getFilePath(),
            {
                {},
                {},
                getMenuElementByTag<ScrollSelectionElement>(
                    PROJECT_SUPPORTED_VERSION)
                    ->getSelectionOption(),
                getMenuElementByTag<TextFieldElement>(PROJECT_NAME)
                    ->getEnteredText(),
                glm::uvec2(getMenuElementByTag<NumberFieldElement>(
                               PROJECT_RESOLUTION_WIDTH)
                               ->getValue(),
                           getMenuElementByTag<NumberFieldElement>(
                               PROJECT_RESOLUTION_HEIGHT)
                               ->getValue()),
                glm::uvec2(
                    getMenuElementByTag<NumberFieldElement>(PROJECT_PIXEL_WIDTH)
                        ->getValue(),
                    getMenuElementByTag<NumberFieldElement>(
                        PROJECT_PIXEL_HEIGHT)
                        ->getValue()),
                getMenuElementByTag<SwitchElement>(PROJECT_VSYNC)->getState(),
                getMenuElementByTag<SwitchElement>(PROJECT_SHOW_CURSOR)
                    ->getState(),
            }};
