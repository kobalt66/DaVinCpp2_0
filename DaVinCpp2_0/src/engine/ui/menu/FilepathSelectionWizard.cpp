...
void FilepathSelectionWizard::interact(SelectionMenu* selectionMenu) {
    if (auto directoryEntry =
            dynamic_cast<DirectoryEntryElement*>(m_SelectedElement.get())) {
        std::filesystem::path newFilepath =
            directoryEntry->getDirectoryEntryPath();
        ...
        if (!DaVinCppFileSystem::canReadWrite(newFilepath)) {
            ...
            SelectionMenu::displayDescription(
                Console::fmtTxt(
                    "Failed to access file/directory at ", newFilepath,
...
void FilepathSelectionWizard::updateDirectoryStructure() {
    ...
    std::pair<std::vector<std::filesystem::directory_entry>,
        ...
        directoryContents =
            DaVinCppFileSystem::getContentsOfDirectory(m_CurrentPath);

    addMenuElement(std::make_shared<PageElement>(
        "<Cancle>", SelectionMenu::PAGE_UNDEFINED, true));
    addMenuElement(std::make_shared<BreakElement>());
    addMenuElement(std::make_shared<ActionButton>(
        "<Apply>",
        [this](SelectionMenu* selectionMenu, ActionButton* buttonRef) {
    ...
    addMenuElement(std::make_shared<BreakElement>());
    addMenuElement(std::make_shared<TextElement>(
        Console::fmtTxt("Current directory > ", m_CurrentPath.string(), "/ "),
        true, ALIGN_LEFT, 5));
    addMenuElement(std::make_shared<DirectoryEntryElement>(
        "../", Console::fmtTxt(m_CurrentPath.string(), "/..")));

    for (const auto& directory : directoryContents.first) {
        std::string fileName = directory.path().filename();
        m_MenuElements.emplace_back(std::make_shared<DirectoryEntryElement>(
            Console::fmtTxt(DaVinCppString::findReplaceAll(
                                directory.path().filename().c_str(), "\"", ""),
                            "/"),
            directory.path()));
    }
