    ...
    if (auto directoryEntry = dynamic_cast<DirectoryEntryElement*>(m_SelectedElement.get()))
    {
        ...
        if (!DaVinCppFileSystem::canReadWrite(newFilepath))
        {
            ...
            SelectionMenu::displayDescription(
                Console::fmtTxt("Failed to access file/directory at ",
                                newFilepath,
...
void FilepathSelectionWizard::updateDirectoryStructure()
{
    ...
    std::pair<std::vector<std::filesystem::directory_entry>,
        ...
        directoryContents = DaVinCppFileSystem::getContentsOfDirectory(m_CurrentPath);
    ...
    addMenuElement(
        std::make_shared<ActionButton>("<Apply>",
    ...
    for (const auto& directory : directoryContents.first)
    {
        ...
        m_MenuElements.emplace_back(std::make_shared<DirectoryEntryElement>(
            Console::fmtTxt(
                DaVinCppString::findReplaceAll(directory.path().filename().c_str(), "\"", ""), "/"),
            directory.path()));
    }
    ...
    {
        m_MenuElements.emplace_back(
            std::make_shared<DirectoryEntryElement>(file.path().filename(), file.path()));
    }
