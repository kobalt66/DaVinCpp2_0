...
void FilepathSelectionWizard::interact(SelectionMenu* selectionMenu)
{
	if (auto directoryEntry = dynamic_cast<DirectoryEntryElement*>(m_SelectedElement.get())) {
		...
		if (!DaVinCppFileSystem::canReadWrite(newFilepath)) {
			...
			SelectionMenu::displayDescription(Console::fmtTxt("Failed to access file/directory at ", newFilepath,
...
void FilepathSelectionWizard::updateDirectoryStructure()
{
	...
	addMenuElement(
	    std::make_shared<ActionButton>("<Apply>", [this](SelectionMenu* selectionMenu, ActionButton* buttonRef) {
	...
	addMenuElement(std::make_shared<BreakElement>());
	addMenuElement(std::make_shared<TextElement>(Console::fmtTxt("Current directory > ", m_CurrentPath.string(), "/ "),
	                                             true, ALIGN_LEFT, 5));
	...
	for (const auto& directory : directoryContents.first) {
		...
		m_MenuElements.emplace_back(std::make_shared<DirectoryEntryElement>(
		    Console::fmtTxt(DaVinCppString::findReplaceAll(directory.path().filename().c_str(), "\"", ""), "/"),
		    directory.path()));
	}
