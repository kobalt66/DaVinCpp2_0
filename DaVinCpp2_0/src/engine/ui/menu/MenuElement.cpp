...
, m_UniqueTag(std::move(uniqueTag))
{
}
...
void MenuElement::onRender(bool selected)
{
    ...
    Console::printCenteredText(m_DisplayText,
                               selected ? Console::GREEN_BG_BLACK_FG : Console::GREEN);
#else
    Console::printCenteredText(
        m_DisplayText, selected ? Console::BLACK_GREEN_PAIR : Console::GREEN_BLACK_PAIR, m_CliY);
#endif
}
...
void MenuElement::setCliY(int cliY) { m_CliY = cliY; }

int MenuElement::getCliY() const { return m_CliY; }

std::string_view MenuElement::getUniqueTag() const { return m_UniqueTag; }
}  // namespace davincpp
