...
namespace davincpp {
ScrollSelectionElement::ScrollSelectionElement(
    const std::string& displayText, const std::vector<std::string>& options,
...
void ScrollSelectionElement::onRender(bool selected) {
    ...
    Console::printTextMarginLR(
        ...
        m_Options.empty()
            ? "< >      null "
            : Console::fmtTxt("< >      ", m_Options.at(m_SelectedOption), " "),
...
void ScrollSelectionElement::onUpdate(SelectionMenu* selectionMenu, int input) {
    ...
    if (input == Console::KEY_ARROW_RIGHT) {
        m_SelectedOption +=
            static_cast<int>(m_SelectedOption + 1 < m_Options.size());
        return;
    }
...
void ScrollSelectionElement::setSelectedOption(const std::string& option) {
    ...
    if (foundOption == m_Options.end()) {
        m_SelectedOption = static_cast<int>(
            std::ranges::distance(m_Options.begin(), foundOption));
    } else {
        m_SelectedOption = 0;
    }
}
