...
void ProjectManager::loadProjectList() {
    std::string configFilePath =
        DaVinCppFileSystem::prepareFilePath(m_DaVinCppConfig.data());

    try {
        if (!DaVinCppFileSystem::exists(configFilePath)) {
            Console::err("Failed to load davincpp config file at '",
                         configFilePath,
        ...
        m_DaVinCppVersion = DaVinCppYamlHelper::getValue<std::string>(
            davincppConfig, "davincppVersion");
        m_LegacyVersions = DaVinCppYamlHelper::getSequence<std::string>(
            davincppConfig, "legacyVersions");
        m_Projects = DaVinCppYamlHelper::getSequence<Project>(
            davincppConfig, "registeredProjects");

        for (Project& project : m_Projects) {
            project.loadProjectConfig(m_DaVinCppVersion);
        }
    } catch (std::runtime_error& error) {
        throw davincpp_error(Console::fmtTxt("Failed to parse yaml file at '",
                                             configFilePath, "': \n",
                                             error.what()));
    }
}
...
std::filesystem::path
ProjectManager::initProject(const ProjectConfig&  projectConfig,
                            std::filesystem::path projectDirectory) {
    std::filesystem::path projectConfigFilePath = projectDirectory.concat(
        Console::fmtTxt(projectConfig.ProjectName, ".yaml"));
