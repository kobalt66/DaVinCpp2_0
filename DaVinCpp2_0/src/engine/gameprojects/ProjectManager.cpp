...
void ProjectManager::loadProjectList()
{
    ...
    try
    {
        ...
        m_DaVinCppVersion
            = DaVinCppYamlHelper::getValue<std::string>(davincppConfig, "davincppVersion");
        m_LegacyVersions
            = DaVinCppYamlHelper::getSequence<std::string>(davincppConfig, "legacyVersions");
    ...
    catch (std::runtime_error& error)
    {
        throw davincpp_error(Console::fmtTxt(
            "Failed to parse yaml file at '", configFilePath, "': \n", error.what()));
    }
}

std::filesystem::path ProjectManager::initProject(const ProjectConfig&  projectConfig,
                                                  std::filesystem::path projectDirectory)
{
    std::filesystem::path projectConfigFilePath
        = projectDirectory.concat(Console::fmtTxt(projectConfig.ProjectName, ".yaml"));
...
std::string ProjectManager::getDaVinCppVersion() const { return m_DaVinCppVersion; }

const std::vector<Project>& ProjectManager::getProjectList() const { return m_Projects; }
