...
bool convert<davincpp::ProjectConfig>::decode(const Node& node, davincpp::ProjectConfig& rhs)
{
	...
	rhs.PixelSize = davincpp::DaVinCppYamlHelper::getValue(
	    node, "pixelSize", glm::uvec2(davincpp::DEFAULT_PIXEL_SIZEX, davincpp::DEFAULT_PIXEL_SIZEY));
	rhs.ScreenResolution = davincpp::DaVinCppYamlHelper::getValue(
	    node, "resolution", glm::uvec2(davincpp::DEFAULT_RESOLUTION_WIDTH, davincpp::DEFAULT_RESOLUTION_HEIGHT));
