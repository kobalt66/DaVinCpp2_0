...
bool DavScript::operator==(const DavScript& other) const { return m_Location == other.m_Location; }
...
bool DavScript::isEmpty() const { return m_FileContent.empty() || m_LineCharacterOffsets.empty(); }
...
bool DavScript::atEndOfFile(CharPosition position) const
{
    return position.getLine() >= m_LineCharacterOffsets.size()
           || (position.getLine() == m_LineCharacterOffsets.size() - 1 && atEndOfLine(position));
}
...
namespace YAML
{
bool convert<davincpp::davscript::DavScript>::decode(const Node&                     node,
                                                     davincpp::davscript::DavScript& rhs)
{
