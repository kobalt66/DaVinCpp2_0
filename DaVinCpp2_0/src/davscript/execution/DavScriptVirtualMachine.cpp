...
DavScriptVirtualMachine::DavScriptVirtualMachine(
...
void DavScriptVirtualMachine::prepareVM() {
    ...
    // todo: save all of the binary file entries. Get the "main" file and start
    // executing code from there. If the code encounters a symbol that is
    // defined in a different file, access it's binary file and execute it.
...
void DavScriptVirtualMachine::execute() {
    try {
        ...
        m_RuntimeErrorMessages.emplace_back(
            DavScriptErrorFormatter::generateRuntimeUnexpectedError(e.what()));
        m_ExitCode = FATAL_ERROR;
    }

    if (m_ExitCode != ZERO) {
        m_RuntimeErrorMessages.emplace_back(
            DavScriptErrorFormatter::generateRuntimeExitCode(m_ExitCode));
...
Value DavScriptVirtualMachine::popStackValue() {
    if (m_Stack.empty()) {
        throw DavScriptException(
            DavScriptErrorFormatter::
                generateRuntimeErrorInvalidStackAccessEmptyStack());
    }
...
Value DavScriptVirtualMachine::readMemory(uint32_t ptr) const {
    if (ptr >= m_Memory.size()) {
        throw DavScriptException(
            DavScriptErrorFormatter::
                generateRuntimeErrorInvalidMemoryReadAccess(ptr));
    }
...
void DavScriptVirtualMachine::writeMemory(uint32_t ptr, const Value& value) {
    if (ptr >= m_Memory.size()) {
        throw DavScriptException(
            DavScriptErrorFormatter::
                generateRuntimeErrorInvalidMemoryWriteAccess(ptr));
    }
...
void DavScriptVirtualMachine::processFunctionCallOperation() {
    ...
    if (!m_RegisteredLibraryFunctions.contains(functionPtr)) {
        throw DavScriptException(
            DavScriptErrorFormatter::generateRuntimeErrorInvalidPointerAccess(
                functionPtr, PTR_CONTEXT_FUNCTION));
    }
...
void DavScriptVirtualMachine::useNamespace(std::string_view namespaceName) {
    if (!DAVSCRIPT_LIBRARIES.contains(namespaceName.data()) ||
        std::ranges::find(m_UsedNamespaces, namespaceName.data()) !=
            m_UsedNamespaces.end()) {
        return;
    }

    for (const auto& symbol :
         DAVSCRIPT_LIBRARIES.at(namespaceName.data()).registeredSymbols |
             std::views::values) {
...
void DavScriptVirtualMachine::logRuntimeErrorInvalidOperation(
    uint8_t operation) {
    m_RuntimeErrorMessages.push_back(
        DavScriptErrorFormatter::generateRuntimeErrorInvalidOperation(
            operation));
}
