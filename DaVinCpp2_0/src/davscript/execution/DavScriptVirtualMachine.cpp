...
void DavScriptVirtualMachine::prepareVM()
{
    ...
    // todo: save all of the binary file entries. Get the "main" file and start executing code from
    // there. If the code encounters a symbol that is defined in a different file, access it's
...
Value DavScriptVirtualMachine::readMemory(uint32_t ptr) const
{
    ...
    {
        throw DavScriptException(
            DavScriptErrorFormatter::generateRuntimeErrorInvalidMemoryReadAccess(ptr));
    }
...
void DavScriptVirtualMachine::writeMemory(uint32_t ptr, const Value& value)
{
    ...
    {
        throw DavScriptException(
            DavScriptErrorFormatter::generateRuntimeErrorInvalidMemoryWriteAccess(ptr));
    }
...
void DavScriptVirtualMachine::processFunctionCallOperation()
{
    ...
    {
        throw DavScriptException(DavScriptErrorFormatter::generateRuntimeErrorInvalidPointerAccess(
...
void DavScriptVirtualMachine::useNamespace(std::string_view namespaceName)
{
    ...
    for (const auto& symbol :
         DAVSCRIPT_LIBRARIES.at(namespaceName.data()).registeredSymbols | std::views::values)
    {
        if (symbol->getSymbolType() == SymbolType::FUNCTION)
        {
            const auto functionSymbol = symbol->castToSymbolType<DavScriptFunctionSymbol>();
            m_RegisteredLibraryFunctions.emplace(functionSymbol->getFunctionPtr(),
...
void DavScriptVirtualMachine::advanceOperationPtrByN(size_t n) { m_OperationPtr += n - 1; }
...
void DavScriptVirtualMachine::logRuntimeErrorInvalidOperation(uint8_t operation)
{
    m_RuntimeErrorMessages.push_back(
        DavScriptErrorFormatter::generateRuntimeErrorInvalidOperation(operation));
}
