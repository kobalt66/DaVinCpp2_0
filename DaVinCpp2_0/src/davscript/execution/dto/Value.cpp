...
Value::Value() : type(ValueType::NONE), data({}) { data.object_t = nullptr; }

Value::Value(int64_t value) : type(ValueType::INT), data({}) {
    data.int_t = value;
}

Value::Value(bool value) : type(ValueType::BOOL), data({}) {
    data.bool_t = value;
}

Value::Value(double value) : type(ValueType::DOUBLE), data({}) {
    data.double_t = value;
}

Value::Value(const std::string& value) : type(ValueType::STRING), data({}) {
    data.string_t = strdup(value.c_str());
}

Value::Value(void* value) : type(ValueType::OBJECT), data({}) {
    data.object_t = value;
}
