...
{
DavScriptCompiler::DavScriptCompiler(std::shared_ptr<Ast> ast, const std::vector<IdentifierNode>& usedNamespaces,
...
void DavScriptCompiler::saveByteCode()
{
	...
	DaVinCppFileSystem::writeFile(
	    binDirectory.append(m_Ast->getModuleNamespace().getName().getActualValue()).append(".bin"), output);
}
	...
	for (const auto& [symbolName, symbol] : registeredSymbols) {
		if (symbol->getSymbolType() == SymbolType::FUNCTION) {
			...
			m_RegisteredLibraryFunctions.emplace(
			    ...
			    std::pair{Console::fmtTxt(namespaceName.getActualValue(), ".", symbolName),
...
void DavScriptCompiler::logInvalidValueTypeError(const Token& typeToken, ValueType expectedType)
{
	m_CompilerErrorMessages.push_back(
	    DavScriptErrorFormatter::generateCompilerErrorInvalidValueType(typeToken, expectedType));
}

void DavScriptCompiler::logFoundAmbiguousFunctionError(const Token& functionName)
{
	m_CompilerErrorMessages.push_back(
	    DavScriptErrorFormatter::generateCompilerErrorFoundAmbiguousFunction(functionName));
}
...
void DavScriptCompiler::checkForCompilationErrors() const
{
	...
	throw std::runtime_error(Console::fmtErr("DavScript failed during compilation!", errorOutput.str(),
	                                         m_CompilerErrorMessages.size(),
