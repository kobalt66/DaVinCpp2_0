...
void DavScriptCompiler::saveByteCode()
{
    ...
    DaVinCppFileSystem::writeFile(
        binDirectory.append(m_Ast->getModuleNamespace().getName().getActualValue()).append(".bin"),
        output);
}
...
void DavScriptCompiler::useNamespace(const Token& namespaceName)
{
    ...
    const auto& registeredSymbols
        = DAVSCRIPT_LIBRARIES.at(namespaceName.getActualValue()).registeredSymbols;
...
void DavScriptCompiler::checkForCompilationErrors() const
{
    ...
    throw std::runtime_error(Console::fmtErr("DavScript failed during compilation!",
                                             errorOutput.str(),
