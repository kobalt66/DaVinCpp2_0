...
std::unordered_map<IdentifierNode, std::shared_ptr<Ast>>
    DavScriptParser::ParsedScriptFilesMap;
std::unordered_map<IdentifierNode, DavScriptNamespace>
    DavScriptParser::RegisteredCustomNamespaces;

DavScriptParser::DavScriptParser(DavScript                 scriptFile,
                                 const std::vector<Token>& tokens, bool root)
    : m_CurrentScript(std::move(scriptFile)), m_Tokens(tokens), m_IsRoot(root) {
    if (m_IsRoot) {
...
void DavScriptParser::mapScriptsToModules(
    ...
    for (const auto& directoryEntry : combinedDirectoryContents) {
        ...
        if (codeLine.empty()) {
            ...
            generatedModuleName             = DaVinCppString::findReplaceAll(
                generatedModuleName,
                Console::fmtTxt(projectDirectory.string(), "/"), "");
            generatedModuleName =
                DaVinCppString::findReplaceAll(generatedModuleName, " ", "_");
            generatedModuleName =
                DaVinCppString::findReplaceAll(generatedModuleName, "/", ".");
            generatedModuleName =
                DaVinCppString::findReplaceAll(generatedModuleName, ".dav", "");
            moduleIdentifier =
                IdentifierNode(Token(script, CharPosition(0, 0),
                                     generatedModuleName, NONE, IDENTIFIER));
        } else {
            ...
            if (moduleNodes.size() > 1) {
                throw DavScriptException(Console::fmtTxt(
                    "Multiple module nodes found in script file: ",
                    script.getName()));
            }
...
void DavScriptParser::skipUntilNextLine() {
    ...
    } while (nextToken.getTokenType() != NEWLINE &&
             nextToken.getTokenRole() != ENDOFFILE);
}
...
bool DavScriptParser::useNamespace(const IdentifierNode& namespaceName) {
    if (DAVSCRIPT_LIBRARIES.contains(
            namespaceName.getName().getActualValue())) {
        ...
        for (const auto& symbol : libraryNamespace.registeredSymbols) {
            m_DefinedSymbols.emplace(
                Console::fmtTxt(namespaceName.getName().getActualValue(), ".",
                                symbol.first),
                symbol.second);
        }
    ...
    // todo: Determine the script file of the custom namespace and parse it.
    // After that, generate a namespace from the script and push it to the
    // m_RegisteredCustomNamespaces map.

    if (RegisteredCustomNamespaces.contains(namespaceName)) {
        DavScriptNamespace customNamespace =
            RegisteredCustomNamespaces.at(namespaceName);
...
bool DavScriptParser::isUsingNamespace(std::string_view namespaceName) const {
    return std::ranges::any_of(
        ...
        [&namespaceName](const IdentifierNode& namespaceNameToken) {
            return namespaceNameToken.getName().getActualValue() ==
                   namespaceName;
        });
}
...
DavScriptNamespace DavScriptParser::getCurrentNamespace() const {
...
bool DavScriptParser::registerSymbol(std::string_view symbolName,
    ...
    m_CurrentNamespace.registeredSymbols.emplace(
        symbolName.data(), m_DefinedSymbols.at(symbolName.data()));
    return true;
}

bool DavScriptParser::validateSymbol(
    const std::shared_ptr<IdentifierNode>& symbolName,
    ...
    return std::ranges::any_of(
        m_UsedNamespaces, [this, &symbolName, &symbolType,
                           &fullName](const IdentifierNode& usedNamespace) {
            std::string namespaceSymbolName = Console::fmtTxt(
                usedNamespace.getName().getActualValue(), ".", fullName);
...
bool DavScriptParser::isValidDefinedSymbol(std::string_view symbolName,
                                           SymbolType       symbolType) const {
    if (!m_DefinedSymbols.contains(symbolName.data())) {
        return false;
    }

    std::shared_ptr<DavScriptSymbol> symbol =
        m_DefinedSymbols.at(symbolName.data());
    return symbol->getScopeDepth() <= m_CurrentScopeDepth &&
           symbol->getSymbolType() == symbolType;
}

bool DavScriptParser::doesVariableAlreadyExist(
    const Token& variableName) const {
    return m_DefinedSymbols.contains(variableName.getActualValue());
}

void DavScriptParser::logUnexpectedTokenError(const Token& actualToken,
                                              const Token& expectedToken) {
    m_ErrorMessages.push_back(
        DavScriptErrorFormatter::generateUnexpectedTokenError(actualToken,
                                                              expectedToken));
}

void DavScriptParser::logInvalidValueTypeError(const Token& valueToken,
                                               TokenType    expectedToken) {
    m_ErrorMessages.push_back(
        DavScriptErrorFormatter::generateInvalidValueTypeError(valueToken,
                                                               expectedToken));
}

void DavScriptParser::logInaccessibleSymbolError(const Token& symbolToken,
                                                 SymbolType   symbolType) {
    m_ErrorMessages.push_back(
        DavScriptErrorFormatter::generateInaccessibleSymbolError(symbolToken,
                                                                 symbolType));
}

void DavScriptParser::logNamespaceNotFoundError(const Token& namespaceToken) {
    m_ErrorMessages.push_back(
        DavScriptErrorFormatter::generateNamespaceNotFoundError(
            namespaceToken));
}

void DavScriptParser::logDuplicateSymbolNameError(const Token& symbolToken,
                                                  SymbolType   symbolType) {
    m_ErrorMessages.push_back(
        DavScriptErrorFormatter::generateDuplicateSymbolNameError(symbolToken,
                                                                  symbolType));
}

void DavScriptParser::logInvalidModuleNamespaceError(
    const Token& moduleNameToken) {
    m_ErrorMessages.push_back(
        DavScriptErrorFormatter::generateInvalidModuleNamespaceError(
            moduleNameToken));
}
...
std::shared_ptr<AstNode>
DavScriptParser::parseKeywords(const Token& nextToken) {
        ...
        case USE: {
            ...
            if (auto useNode = std::dynamic_pointer_cast<UseNode>(node)) {
                if (!useNamespace(*useNode->getNamespaceName())) {
                    logNamespaceNotFoundError(
                        useNode->getNamespaceName()->getName());
                    return std::make_shared<InvalidNode>();
                }
            }
...
DavScriptNamespace
DavScriptParser::parseUsedScriptFile(const IdentifierNode& namespaceName) {
    if (RegisteredCustomNamespaces.contains(namespaceName)) {
    ...
    RegisteredCustomNamespaces[parser.getCurrentNamespaceName()] =
...
void DavScriptParser::checkForErrors() const {
    ...
    if (m_IsRoot) {
        throw std::runtime_error(
            Console::fmtErr("DavScript failed to be parsed!", errorOutput.str(),
                            m_CurrentScript.getName(), ": ", errorCountStr,
                            " occurred during the parsing phase."));
    }

    throw std::runtime_error(
        Console::fmtTxt(errorOutput.str(), m_CurrentScript.getName(), ": ",
                        errorCountStr, " occurred during the parsing phase."));
}
} // namespace davincpp::davscript
