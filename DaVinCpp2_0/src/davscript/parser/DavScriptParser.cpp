...
void DavScriptParser::mapScriptsToModules(const std::filesystem::path& projectDirectory)
{
	...
	std::vector<std::filesystem::directory_entry> davLibsDirectoryContents =
	    DaVinCppFileSystem::getFilesInDirectoryRecursive(DaVinCppFileSystem::prepareFilePath("[DAV_LIBS]"));
	...
	combinedDirectoryContents.insert(combinedDirectoryContents.end(), davLibsDirectoryContents.begin(),
		...
		if (codeLine.empty()) {
			...
			if (moduleNodes.size() > 1) {
				throw DavScriptException(
				    Console::fmtTxt("Multiple module nodes found in script file: ", script.getName()));
			}
...
bool DavScriptParser::useNamespace(const IdentifierNode& namespaceName)
{
	if (DAVSCRIPT_LIBRARIES.contains(namespaceName.getName().getActualValue())) {
		...
		for (const auto& symbol : libraryNamespace.registeredSymbols) {
			m_DefinedSymbols.emplace(Console::fmtTxt(namespaceName.getName().getActualValue(), ".", symbol.first),
			                         symbol.second);
		}
	...
	// todo: Determine the script file of the custom namespace and parse it. After that, generate a namespace from the
	// script and push it to the m_RegisteredCustomNamespaces map.
...
bool DavScriptParser::validateSymbol(const std::shared_ptr<IdentifierNode>& symbolName, SymbolType symbolType) const
{
	...
	return std::ranges::any_of(
	    m_UsedNamespaces, [this, &symbolName, &symbolType, &fullName](const IdentifierNode& usedNamespace) {
...
void DavScriptParser::checkForErrors() const
{
	...
	if (m_IsRoot) {
		throw std::runtime_error(Console::fmtErr("DavScript failed to be parsed!", errorOutput.str(),
		                                         m_CurrentScript.getName(), ": ", errorCountStr,
		                                         " occurred during the parsing phase."));
	}

	throw std::runtime_error(Console::fmtTxt(errorOutput.str(), m_CurrentScript.getName(), ": ", errorCountStr,
	                                         " occurred during the parsing phase."));
}
} // namespace davincpp::davscript
