...
namespace davincpp {
std::shared_ptr<davscript::AstNode>
davscript::ModuleNodeParser::parseNode(DavScriptParser* scriptParser) {
    assert(assertTokenType(scriptParser, scriptParser->advanceToken(),
                           Token(MODULE, KEYWORD)));

    auto moduleName = std::dynamic_pointer_cast<IdentifierNode>(
