...
AssignmentParser::parseNode(DavScriptParser* scriptParser) {
    assert(assertTokenRole(scriptParser, scriptParser->peakNextToken(),
                           Token(NONE, VARIABLETYPE)));
    Token variableType = scriptParser->advanceToken();

    assert(assertTokenRole(scriptParser, scriptParser->peakNextToken(),
                           Token(NONE, IDENTIFIER)));
    auto variableName =
        std::make_shared<IdentifierNode>(scriptParser->advanceToken());

    assert(assertSymbolDoesntExist(scriptParser, variableName,
                                   SymbolType::VARIABLE));

    assert(assertTokenRole(scriptParser, scriptParser->peakNextToken(),
                           Token(NONE, VALUETYPE)));
    ...
    return std::make_shared<AssignmentNode>(variableType,
                                            variableName->getName(), valueType,
