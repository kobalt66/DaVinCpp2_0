...
std::shared_ptr<IdentifierNode> UseNode::getNamespaceName() const { return m_NamespacedName; }
...
std::vector<uint8_t> UseNode::generateByteCode(DavScriptCompiler* compiler)
{
    ...
    std::vector<uint8_t> namespaceName
        = ByteCastHelper::stringToBytes(m_NamespacedName->getName().getActualValue());
