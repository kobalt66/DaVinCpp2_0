...
FunctionCallNode::FunctionCallNode(
    Token functionName, const std::vector<std::shared_ptr<AstNode>>& parameters)
    : m_FunctionName(std::move(functionName)), m_Parameters(parameters) {}
...
std::vector<uint8_t>
FunctionCallNode::generateByteCode(DavScriptCompiler* compiler) {
    std::vector<uint8_t> byteCode;

    std::vector<uint8_t> functionPtr =
        compiler->determineFunctionPtr(m_FunctionName);
    ...
    for (const auto& parameter : m_Parameters) {
        if (auto valueNode = std::dynamic_pointer_cast<ValueNode>(parameter)) {
            ...
            std::vector<uint8_t> valueBytes =
                valueNode->generateByteCode(compiler, valueType);

            switch (valueType.getTokenType()) {
                case INTTYPE:
                    parametersByteCode.push_back(MOV_INT);
                    break;
                case BOOLTYPE:
                    parametersByteCode.push_back(MOV_BOOL);
                    break;
                case FLOATTYPE:
                    parametersByteCode.push_back(MOV_FLOAT);
                    break;
                case STRINGTYPE:
                    parametersByteCode.push_back(MOV_STRING);
                    break;
                default:
                    continue;
            }

            parametersByteCode.insert(parametersByteCode.end(),
                                      valueBytes.begin(), valueBytes.end());
        }
    }

    byteCode.insert(byteCode.begin(), parametersByteCode.begin(),
                    parametersByteCode.end());
