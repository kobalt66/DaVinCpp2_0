...
{
FunctionCallNode::FunctionCallNode(Token                                        functionName,
...
[[nodiscard]] Token FunctionCallNode::getFunctionName() const { return m_FunctionName; }
...
std::vector<uint8_t> FunctionCallNode::generateByteCode(DavScriptCompiler* compiler)
{
    ...
    for (const auto& parameter : m_Parameters)
    {
        if (auto valueNode = std::dynamic_pointer_cast<ValueNode>(parameter))
        {
            ...
            parametersByteCode.insert(
                parametersByteCode.end(), valueBytes.begin(), valueBytes.end());
        }
    }
