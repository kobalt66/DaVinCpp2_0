...
IdentifierNode::IdentifierNode(Token name) : m_Name(std::move(name)) {
...
Token IdentifierNode::getName() const { return m_Name; }

size_t IdentifierNode::getNameSegCount() const { return m_NameSegments.size(); }

Token IdentifierNode::getFullParentSeg() const {
    if (m_NameSegments.empty()) {
        throw DavScriptException(
            "Invalid identifier caught: Identifier is empty");
    }
...
Token IdentifierNode::getLastNameSeg() const {
    ...
    for (size_t i = 0; i < lastSegIndex; i++) {
        segPosition.incrementCharIdx(
            static_cast<int>(m_NameSegments.at(i).size() + 1));
    }
...
std::vector<uint8_t>
IdentifierNode::generateByteCode(DavScriptCompiler* compiler) {
    return {};
}
} // namespace davincpp::davscript
