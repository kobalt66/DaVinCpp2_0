...
{
	m_Tokens.emplace_back(m_DavScript, m_CurrentCharPosition, std::string(1, m_CurrentChar),
...
void DavScriptLexer::lexVariableTypeToken()
{
	...
	m_Tokens.emplace_back(m_DavScript, startPosition, variableTypeValue, VARIABLE_TYPE_TOKENS.at(variableTypeValue),
	                      VARIABLETYPE);
}
...
void DavScriptLexer::lexNumberToken()
{
	...
	m_Tokens.emplace_back(m_DavScript, startPosition, numberStr.str(), hasDecimalPoint ? NUMBERFLOAT : NUMBERINT,
	                      DATAVALUE);
}
...
bool DavScriptLexer::tokenValueOnWhiteList(std::string_view tokenValue,
