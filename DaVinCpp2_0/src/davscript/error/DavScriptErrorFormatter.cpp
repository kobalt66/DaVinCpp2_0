...
std::string DavScriptErrorFormatter::generateUnexpectedTokenError(const Token& actualToken, const Token& expectedToken)
{
	...
	return Console::fmtTxt(generateErrorSeparator(info.size()), info, "Parsing error: Unexpected token:\n",
...
std::string DavScriptErrorFormatter::generateInvalidValueTypeError(const Token& valueToken, TokenType expectedToken)
{
	...
	return Console::fmtTxt(generateErrorSeparator(info.size()), info, "Parsing error: Invalid value type:\n",
...
std::string DavScriptErrorFormatter::generateInaccessibleSymbolError(const Token& symbolToken, SymbolType symbolType)
{
	...
	return Console::fmtTxt(generateErrorSeparator(info.size()), info, "Parsing error: Inaccessible symbol:\n",
...
std::string DavScriptErrorFormatter::generateNamespaceNotFoundError(const Token& namespaceToken)
{
	...
	return Console::fmtTxt(generateErrorSeparator(info.size()), info, "Parsing error: Namespace not found:\n",
...
{
	std::string info = generateErrorLocationInfo(symbolToken.getDavScript(), symbolToken.getTokenPosition());
	return Console::fmtTxt(generateErrorSeparator(info.size()), info, "Parsing error: Symbol name already defined:\n",
	                       generateErrorCodeLine(symbolToken), '\n', "Symbol name: ", symbolToken.getActualValue(),
	                       '\n', "Symbol type: ", SYMBOL_TYPE2STRING.at(symbolType));
}
                                                                           ...
                                                                           ValueType expectedType)
{
	...
	return Console::fmtTxt(generateErrorSeparator(info.size()), info, "Compiler error: Invalid type:\n",
...
std::string DavScriptErrorFormatter::generateCompilerErrorFoundAmbiguousFunction(const Token& functionName)
{
	...
	return Console::fmtTxt(generateErrorSeparator(info.size()), info, "Compiler error: Found ambiguous function:\n",
...
std::string DavScriptErrorFormatter::generateRuntimeErrorInvalidParameterValue(std::string_view functionName,
                                                                               int parameterIndex, ValueType actualType,
                                                                               std::string_view expectedType)
{
	return Console::fmtTxt("Runtime error: Invalid parameter value for '", functionName, "' at the ", parameterIndex,
	                       ". parameter: Expected ", expectedType, ", got ", STACK_VALUE_TYPE2STRING.at(actualType),
	                       "");
}
...
{
	return Console::fmtTxt("Script: ", davScript.getLocation(), " (ln: ", position.getLine() + 1,
...
std::string DavScriptErrorFormatter::generateErrorCodeLine(const Token& token)
{
	...
	return Console::fmtTxt(lineNumber, " | ", cleansedLine, '\n', std::string(lineNumber.length(), ' '), " | ",
