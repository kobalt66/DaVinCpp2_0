...
{
std::string DavScriptErrorFormatter::generateUnexpectedTokenError(const Token& actualToken,
                                                                  const Token& expectedToken)
{
    std::string info
        = generateErrorLocationInfo(actualToken.getDavScript(), actualToken.getTokenPosition());
    return Console::fmtTxt(generateErrorSeparator(info.size()),
                           info,
...
std::string DavScriptErrorFormatter::generateInvalidValueTypeError(const Token& valueToken,
                                                                   TokenType    expectedToken)
{
    std::string info
        = generateErrorLocationInfo(valueToken.getDavScript(), valueToken.getTokenPosition());
    return Console::fmtTxt(generateErrorSeparator(info.size()),
                           info,
                           ...
                           '\n',
                           "Actual value type: ",
                           TOKEN_TYPE2STRING.at(valueToken.getTokenType()),
                           '\n',
                           "Expected value type: ",
                           TOKEN_TYPE2STRING.at(expectedToken));
}

std::string DavScriptErrorFormatter::generateInaccessibleSymbolError(const Token& symbolToken,
                                                                     SymbolType   symbolType)
{
    std::string info
        = generateErrorLocationInfo(symbolToken.getDavScript(), symbolToken.getTokenPosition());
    return Console::fmtTxt(generateErrorSeparator(info.size()),
                           info,
                           ...
                           '\n',
                           "Symbol name: ",
                           symbolToken.getActualValue(),
                           '\n',
                           "Symbol type: ",
                           SYMBOL_TYPE2STRING.at(symbolType));
}

std::string DavScriptErrorFormatter::generateNamespaceNotFoundError(const Token& namespaceToken)
{
    std::string info = generateErrorLocationInfo(namespaceToken.getDavScript(),
                                                 namespaceToken.getTokenPosition());
    return Console::fmtTxt(generateErrorSeparator(info.size()),
                           info,
                           ...
                           '\n',
                           "Namespace name: ",
                           namespaceToken.getActualValue());
}

std::string DavScriptErrorFormatter::generateDuplicateSymbolNameError(const Token& symbolToken,
                                                                      SymbolType   symbolType)
{
    std::string info
        = generateErrorLocationInfo(symbolToken.getDavScript(), symbolToken.getTokenPosition());
...
{
    std::string info = generateErrorLocationInfo(moduleNameToken.getDavScript(),
                                                 moduleNameToken.getTokenPosition());
    return Console::fmtTxt(
        ...
        "Module namespace: ",
        moduleNameToken.getActualValue());
}

std::string DavScriptErrorFormatter::generateCompilerErrorInvalidValueType(const Token& typeToken,
                                                                           ValueType expectedType)
{
    std::string info
        = generateErrorLocationInfo(typeToken.getDavScript(), typeToken.getTokenPosition());
    return Console::fmtTxt(generateErrorSeparator(info.size()),
                           info,
                           ...
                           '\n',
                           "Actual type: ",
                           typeToken.getActualValue(),
                           '\n',
                           "Expected type: ",
                           STACK_VALUE_TYPE2STRING.at(expectedType));
}

std::string DavScriptErrorFormatter::generateCompilerErrorFoundAmbiguousFunction(
    const Token& functionName)
{
    std::string info
        = generateErrorLocationInfo(functionName.getDavScript(), functionName.getTokenPosition());
    return Console::fmtTxt(generateErrorSeparator(info.size()),
                           info,
...
{
    std::stringstream ss;
    ss << "0x" << std::setfill('0') << std::setw(4) << std::hex << static_cast<uint32_t>(operation);
...
std::string DavScriptErrorFormatter::generateErrorLocationInfo(const DavScript& davScript,
                                                               CharPosition     position)
{
    return Console::fmtTxt("Script: ",
                           davScript.getLocation(),
                           " (ln: ",
                           position.getLine() + 1,
                           ", char: ",
                           position.getCharIdx() + 1,
                           "):\n");
}
...
std::string DavScriptErrorFormatter::generateErrorCodeLine(const Token& token)
{
    ...
    return Console::fmtTxt(lineNumber,
                           " | ",
                           cleansedLine,
                           '\n',
