...
void Console::printCenteredText(std::string_view text, const char* color, char firstChar, char lastChar)
{
	...
	std::cout << color;
	std::cout << firstChar << std::setw(padding) << text.data() << std::setw(consoleWidth - padding - 1) << lastChar;
	std::cout << "\n";
	std::cout << "\033[0m";
}
...
void Console::printText(std::string_view text, int colorPair, int cliY)
{
	...
	mvhline(cliY, length + 1, ' ', maxx - text.size() - 1);
	attroff(COLOR_PAIR(colorPair));
	refresh();
}
...
void Console::printTextMarginLR(std::string_view textLeft, std::string_view textRight, int colorPair, int cliY,
