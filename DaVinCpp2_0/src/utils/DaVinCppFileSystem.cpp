...
void DaVinCppFileSystem::writeFile(const std::filesystem::path& path, std::string_view data)
{
	...
	if (!canWrite(targetDirectory)) {
		throw system_error(Console::fmtTxt("Failed to write data to file at ", finalPath,
	...
	if (!fileStream.is_open()) {
		throw system_error(
		    Console::fmtTxt("Something went wrong while trying to access the target file at ", finalPath, "!"));
	}
...
std::string DaVinCppFileSystem::readFile(const std::filesystem::path& path)
{
	...
	if (!canRead(finalPath)) {
		throw system_error(Console::fmtTxt("Failed to read data from file at ", finalPath,
...
std::pair<std::vector<std::filesystem::directory_entry>, std::vector<std::filesystem::directory_entry>>
DaVinCppFileSystem::getContentsOfDirectory(const std::filesystem::path& directoryPath)
{
	std::filesystem::path finalPath = prepareFilePath(directoryPath);

	if (!exists(finalPath)) {
		throw system_error(
		    Console::fmtTxt("Failed to read data to directory at ", finalPath, ": The directory path is invalid!"));
	}
...
std::vector<std::filesystem::directory_entry> DaVinCppFileSystem::getFilesInDirectoryRecursive(
