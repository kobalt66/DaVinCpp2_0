...
void DaVinCppFileSystem::writeFile(const std::filesystem::path& path, std::string_view data)
{
    ...
    {
        throw system_error(Console::fmtTxt("Failed to write data to file at ",
                                           finalPath,
    ...
    if (!fileStream.is_open())
    {
        throw system_error(Console::fmtTxt(
            "Something went wrong while trying to access the target file at ", finalPath, "!"));
    }
...
std::string DaVinCppFileSystem::readFile(const std::filesystem::path& path)
{
    ...
    {
        throw system_error(Console::fmtTxt("Failed to read data from file at ",
                                           finalPath,
...
DaVinCppFileSystem::getContentsOfDirectory(const std::filesystem::path& directoryPath)
{
    ...
    {
        throw system_error(Console::fmtTxt(
            "Failed to read data to directory at ", finalPath, ": The directory path is invalid!"));
    }
