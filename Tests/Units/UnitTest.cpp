    ...
    for (const auto& [testStepName, testStep] : m_TestSteps)
    {
        ...
        catch (std::exception& exception)
        {
            ...
            if (!m_ExpectedException.empty())
            {
                ...
                std::string failedTestDescription
                    = Console::fmtTxt("\nTest step '",
                                      "testAssignmentNodeFailure()",
                                      "' failed: \n",
...
int UnitTest::getTestStepCount() const { return static_cast<int>(m_TestSteps.size()); }

const std::vector<TestResult>& UnitTest::getTestResult() const { return m_TestResults; }
